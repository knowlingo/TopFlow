...
function JavaScriptNode({ data, selected }: NodeProps<Node<JavaScriptNodeData>>) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 border-b border-border/50 bg-gradient-to-r from-amber-500/10 to-transparent px-2.5 py-1.5">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-500 shadow-sm">
          <Code className="h-3.5 w-3.5 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-foreground leading-tight">JavaScript</h3>
          <p className="text-xs text-muted-foreground/90 truncate leading-tight mt-0.5">Execute custom code</p>
        </div>
        <Settings className="h-4 w-4 text-muted-foreground/70 shrink-0" />
      </div>

      <div className="px-2.5 py-1.5">
        <div className="rounded-md bg-background/90 px-2.5 py-2 border border-border/30">
          <pre className="text-xs text-muted-foreground/90 overflow-x-auto max-h-16 leading-tight">
            {data.code || "// Enter JavaScript code..."}
          </pre>
        </div>
        {status === "running" && (
          <div className="mt-2 flex items-center gap-2 text-xs text-yellow-600 dark:text-yellow-500 font-medium">
      ...
      <Handle
        ...
        id="input"
        className="!bg-amber-500 !w-3 !h-3 !border-2 !border-background shadow-lg"
      />
      <Handle
        type="source"
